...
for(year in years){
  vat_threshold_count<-sum(vat_thresholds$key[vat_thresholds$year==year],na.rm = T)
  put(vat_threshold_count)
}
vat_threshold_count<-magic_result_as_dataframe() 

vat_threshold_avg<-merge(vat_threshold_avg,vat_threshold_count,by="year")
colnames(vat_threshold_avg)[colnames(vat_threshold_avg)=="vat_threshold_count"] <- "threshold_observations"
